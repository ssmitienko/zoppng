...
func (e *encoder) writeImage(w io.Writer, m image.Image, cb int) error {
	...
	for y := b.Min.Y; y < b.Max.Y; y++ {
		...
		*/
		...
		for j := 0; j < len(cr[f]); j++ {
			byteSlice = append(byteSlice, cr[f][j])
		}
	...
	return zopfli.ZlibCompress(&options, byteSlice, w)
}
...
func (e *encoder) writeIDATs() {
	...
	e.err = e.writeImage(e.bw, e.m, e.cb)
	if e.err != nil {
		return
	}
	e.err = e.bw.Flush()
